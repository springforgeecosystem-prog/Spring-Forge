...
class DockerfilePromptBuilder {
    ...
    private fun buildIntellijPluginPrompt(projectInfo: ProjectInfo): String {
        ...
        return buildString {
            ...
            appendLine("- Install bash: RUN apk add --no-cache bash")
            ...
            appendLine("- FROM eclipse-temurin:${projectInfo.javaVersion}-jdk-alpine AS builder")
            appendLine("- Install ONLY bash (no gradle - wrapper exists!)")
            appendLine("- WORKDIR /build")
            ...
            appendLine("✓ FROM eclipse-temurin:${projectInfo.javaVersion}-jdk-alpine AS builder")
            appendLine("✓ Bash installation: RUN apk add --no-cache bash")
            appendLine("✓ NO gradle installation (only bash)")
